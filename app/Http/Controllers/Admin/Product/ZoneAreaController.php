...
use App\Http\Services\Admin\Product\ZoneAreaServices;
use Validator;
...
class ZoneAreaController extends Controller
{
    ...
    $this->service = new ZoneAreaServices();
    }

    public function index(){
        try {
            $zonearea = $this->service->getAll();
            return view('admin.pages.product.zone-area.list-zone-area', compact('zonearea'));
        } catch (\Exception $e) {
            return $e;
        }
    }
    ...
    public function add(){
        return view('admin.pages.product.zone-area.add-zone-area');
    }
    ...
    public function store(Request $request){
        ...
        $rules = [
            ...
            'image' => 'required|image|mimes:jpeg,png,jpg|max:' . Config::get("AllFileValidation.ZONE_AREA_IMAGE_MAX_SIZE") . '|min:' . Config::get("AllFileValidation.ZONE_AREA_IMAGE_MIN_SIZE"),
        ...
        try {
            ...
            if ($validation->fails()) {
                ...
                if ($add_data) {
                    ...
                    if ($status == 'success') {
                        return redirect('list-zone-area')->with(compact('msg', 'status'));
                    } else {
                        return redirect('add-zone-area')->withInput()->with(compact('msg', 'status'));
                    }
                }
            }
        } catch (Exception $e) {
            return redirect('add-zone-area')->withInput()->with(['msg' => $e->getMessage(), 'status' => 'error']);
        }
    }

    public function show(Request $request){
        try {
            $zonearea = $this->service->getById($request->show_id);
          
            return view('admin.pages.product.zone-area.show-zone-area', compact('zonearea'));
        } catch (\Exception $e) {
            return $e;
        }
    }
    ...
    public function edit(Request $request){
        ...
        $zonearea = $this->service->getById($edit_data_id);
        return view('admin.pages.product.zone-area.edit-zone-area', compact('zonearea'));
    }
    ...
    public function update(Request $request){
        ...
        if($request->has('image')) {
            $rules['image'] = 'required|image|mimes:jpeg,png,jpg|max:'.Config::get("AllFileValidation.ZONE_AREA_IMAGE_MAX_SIZE").'|min:'.Config::get("AllFileValidation.ZONE_AREA_IMAGE_MIN_SIZE");
        }
    ...
    public function updateOne(Request $request){
        ...
        $result = $this->service->updateOne($active_id);
            return redirect('list-zone-area')->with('flash_message', 'Updated!');  
        } catch (\Exception $e) {
            return $e;
        }
    }
